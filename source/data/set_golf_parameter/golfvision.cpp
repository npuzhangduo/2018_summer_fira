 ...
 *      2.确认头文件和cpp里面Imgprocess函数是否更改 void Golf::imageProcess(cv::Mat image, ImgProcResult *Result)
 *      3.确认头文件中的类名是否更改class Golf : public ImgProc
 *      4.确认类中是否增加成员变量GolfResult MyResult;
 *      5.确认头文件中是否增加class GolfResult : public ImgProcResult
 *      6.确认imgprocess函数中是否给Result类返回参数信息
 *
 *        MyResult.center.x = center_cols;
 *        MyResult.center.y = center_rows;
 *        MyResult.valid    = false;
 *        (*dynamic_cast<GolfResult*>(Result)) = MyResult;
 *
 *      7.是否在imgprocess函数末尾增加 (*dynamic_cast<GolfResult*>(Result)) = MyResult;
 *      8.是否更改了调试时图片的输出路径
...
/******Public******/
//void Golf::imageProcess(cv::Mat image, ImgProcResult *Result)
void Golf::imageProcess(cv::Mat image)
{
    ball_setNotFound();
    hole_setNotFound();
    ...
    /*******************************************/
    if (ball_rect.x != -1) {
        rectangle(center_img, ball_rect, cv::Scalar(0, 0, 255), 2, 8);
    ...
    if (hole_rect.x != -1) {
        rectangle(center_img, hole_rect, cv::Scalar(255, 0, 0), 2, 8);
...
void Golf::Load_std_img()
{
//    std_hole    = imread("D:/Robot/HuroCup 2017/Prepare/Golf/std_img/1/y.png"); //yellow
//    std_ball = imread("D:/Robot/HuroCup 2017/Prepare/Golf/std_img/1/r1.png");//red

    // std_hole= cv::imread("/home/zenghao/HuroCup 2017/Prepare/Golf/std_img/1/y1.png");
//    std_hole    = imread("/home/zenghao/HuroCup 2017/Prepare/Golf/std_img/1/y.png"); //yellow
//    std_ball = imread("/home/zenghao/HuroCup 2017/Prepare/Golf/std_img/1/r1.png");//red
...
void Golf::Pretreat(cv::Mat image)
{
    //basket
    src_img = image.clone();
...
cv::Mat Golf::getHistogram_Image(const cv::Mat &image)
{
...
void Golf::Write_img(cv::Mat& src,  const char floder_name[30], int num)
{
...
void Golf::Process_hole(cv::Mat image)
{
...
void Golf::Find_hole_center(cv::Mat src)
{
...
int Golf::Judge_hole_legal(cv::Mat& src)
{
...
cv::Mat Golf::hole_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void Golf::Process_ball(cv::Mat image)
{
    image = channel[0].clone();
...
void Golf::Find_ball_center(cv::Mat src)
{
...
int Golf::Judge_ball_legal(cv::Mat& src)
{
    if (src.rows >= 100 || src.cols >= 100 || src.rows <= 10 || src.cols <= 10)
        return 0;
    else
        return 1;
}

cv::Mat Golf::ball_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
int Golf::Judge_centerInRect(Point2f center, Rect rect)
{
...
double Golf::Count_possible(Mat bianry_img, Rect roi_rect)
{
...
//new
void Golf::Load_parameter()
{
//read
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/Golf/test_img/2.txt");
...
void Golf::Store_parameter()
{
//write

//    ofstream out("D:/Robot/HuroCup 2017/Prepare/Golf/test_img/2.txt");
    ofstream out("3.txt");
    if(out.is_open()){
        printf("is_open\n");
        out << hole_h_min_thre    << " ";
