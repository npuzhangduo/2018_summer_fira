...
//void FiraObstacleLine::imageProcess(cv::Mat image)
{
	...
	//printf("line_rect.y::%lf, line_rect.x::%lf\n",line_rect.y, line_rect.x);
        line_center_rows = line_rect.y+line_rect.height*0.5;
        line_center_cols = line_rect.x+line_rect.width*0.5;
	//printf("line_center_rows::%lf, line_center_cols::%lf\n",line_center_rows, line_center_cols);
...
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/FiraObstacleLine/test_img/2.txt");
    ifstream ifile("../source/data/set_fira_obstacle_line_parameter/3.txt");
    if(!ifile){
        cerr<<"error"<<endl;
    }
    string lineword;
