...
//    Set_parameter();
    ...
    if (color2_rect.x != -1)
    {
        ...
        color2_area = color2_rect.height * color2_rect.width;
        color2_valid = true;
        color2_height = color2_rect.height;
    }
    else
        color2_valid = false;
    ...
    if (color1_rect.x != -1)
    {
        ...
        color1_area = color1_rect.height * color1_rect.width;
        color1_height = color1_rect.height;
        color1_valid = true;
    }
    else
        color1_valid = false;
    ...
    if (color3_rect.x != -1)
    {
        ...
        color3_area = color3_rect.height * color3_rect.width;
        color3_height = color3_rect.height;
        color3_valid = true;
    }
    else
        color3_valid = false;
    ...
    MyResult.color1_center.x = color1_center_cols;
    MyResult.color1_center.y = color1_center_rows;
    MyResult.color1_area = color1_area;
    MyResult.color1_valid    = color1_valid;
    MyResult.color1_height = color1_height;

    MyResult.color2_center.x = color2_center_cols;
    MyResult.color2_center.y = color2_center_rows;
    MyResult.color2_area = color2_area;
    MyResult.color2_valid    = color2_valid;
    MyResult.color2_height = color2_height;

    MyResult.color3_center.x = color3_center_cols;
    MyResult.color3_center.y = color3_center_rows;
    MyResult.color3_area = color3_area;
    MyResult.color3_valid    = color3_valid;
    MyResult.color3_height = color3_height;

    MyResult.line_center.x = line_center_row;
    MyResult.line_center.y = line_center_col;
    MyResult.line_angle = line_angle;
...
//    std_color2 = imread("/home/zenghao/HuroCup 2017/Prepare/FiraObstacleVision/std_img/1/r1.png");//red
    ...
    std_color1    = imread("../source/data/std_img/y.png"); //yellow
    std_color2 = imread("../source/data/std_img/r1.png");//red
...
void FiraObstacleVision::Process_color2(cv::Mat image)
{
    ...
    Mat thre_s;
    threshold(channel[1], thre_s, color2_s_thre, 255, cv::THRESH_BINARY);

    color2_project_result = channel[0].clone();
    for(int i = 0; i < channel[0].rows; i++){
        for(int j = 0; j < channel[0].cols; j++){
            int value = channel[0].at<uchar>(i,j);
            if(value >= color2_h_min_thre && value <= color2_h_max_thre){
    ...
    //erode and dilate
    Erode_img(color2_project_result, color2_project_result, 1);
    Dilate_img(color2_project_result, color2_project_result, 1);

    bitwise_and(color2_project_result, thre_s, color2_project_result);
...
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/FiraObstacleVision/test_img/2.txt");
    ifstream ifile("../source/data/set_fira_obstacle_parameter/3.txt");
    if(!ifile){
        cerr<<"error"<<endl;
    }
    string lineword;
