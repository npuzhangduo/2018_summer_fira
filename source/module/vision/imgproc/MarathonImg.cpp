...
//    Process_hole(image);
    ...
    /*******************************************/
    if (line_rect.x != -1) {
        line_center_rows = mass_center.y;
        line_center_cols = mass_center.x;
        line_valid = true;

        for(int i = 0; i < block_num; i++)
        {
            circle(center_img, part_masscenter[i], 2, cv::Scalar(0, 0, 0), 2, 8, 0);
        }
        circle(center_img, cv::Point2f(line_center_cols, line_center_rows), 2, cv::Scalar(0, 0, 255), 6, 8, 0);
...
//////////////////////////////////////////
    ...
    MyResult.line_center.x    = line_center_cols;
    MyResult.line_center.y    = line_center_rows;
    for (int i = 0;i<4;i++)
    MyResult.part_line_center[i] = part_masscenter[i];
    MyResult.line_valid       = line_valid;
    (*dynamic_cast<MarathonResult*>(Result)) = MyResult;
}
...
void Marathon::Find_line_center(cv::Mat src)
{
    ...
    if (contours.size() > 0)
    {
        ...
        if (most_possible != -1)
        {
            ...
            mass_center = Find_mass_center(src, line_rect);
        }
        else
        {
            line_setNotFound();
...
cv::Point2f Marathon::Find_mass_center(Mat &bianry_img, Rect roi_rect)
{
    ...
    for(int i = 0; i < block_num; i++)
    {
        part_masscenter[i] = cv::Point2f(1.0*block_col_sum[i]/block_cnt[i], 1.0*block_row_sum[i]/block_cnt[i]);
...
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/Marathon/test_img/2.txt");
    ifstream ifile("../source/data/set_marathon_parameter/3.txt");
    if(!ifile){
        cerr<<"error"<<endl;
    }
    string lineword;
