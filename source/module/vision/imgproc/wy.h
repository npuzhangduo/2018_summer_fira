...
class wyResult : public ImgProcResult
{
public:
    cv::Point2f center;
...
public:
    ...
    {
        wyResult *tmp = dynamic_cast<wyResult*>(&res);
        center = tmp->center;
        obj_valid = tmp->obj_valid;
    }

    void operator = (wyResult &res)
    {
        center = res.center;
...
public:
    ...
    int Judge_big_legal(cv::Rect rec);
