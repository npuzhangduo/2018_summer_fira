...
/******Public******/
void Fira2v2Vision::imageProcess(cv::Mat image, ImgProcResult *Result)
 // void Fira2v2Vision::imageProcess(cv::Mat image)
{
     Not_Find_Basket();
     Not_Find_Ball();
    debug_print = 0;
...
//    waitKey(0);
   ...
   (*dynamic_cast<Fira2v2VisionResult*>(Result)) = MyResult;

}

void Fira2v2Vision::Load_std_img()
{
...
//    imshow("1", std_basket_hsv_channel[0]);
    std_basket_hist = getFira2v2Vision(std_basket_hsv_channel[0]);
    std_ball_1_hist = getFira2v2Vision(std_ball_1_channel[0]);
    std_ball_2_hist = getFira2v2Vision(std_ball_2_channel[0]);
...
void Fira2v2Vision::Pretreat(cv::Mat image)
{
    //basket
    src_img = image.clone();
...
void Fira2v2Vision::Dilate_img(Mat src, Mat &dst, int size)
{
    Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    dilate(src, dst, element, Point(-1, -1), 1);
}

void Fira2v2Vision::Erode_img(cv::Mat src, cv::Mat &dst, int size)
{
    cv::Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    erode(src, dst, element, Point(-1, -1), 1);
}

cv::MatND Fira2v2Vision::getFira2v2Vision(const cv::Mat &image)
{
    cv::MatND hist;//创建直方图
    cv::calcHist(//计算直方图
        &image,//待计算的图像
...
cv::Mat Fira2v2Vision::getFira2v2Vision_Image(const cv::Mat &image)
{
    cv::MatND hist = getFira2v2Vision(image);//首先计算直方图
...
void Fira2v2Vision::Write_img(cv::Mat& src,  const char floder_name[30], int num)
{
...
int Fira2v2Vision::Judge_center_close()
{
...
void Fira2v2Vision::Process_Basket(cv::Mat image)
{
    image = channel[0].clone();

	basket_project_result = image;
...
void Fira2v2Vision::Find_Basket_center(cv::Mat src)
{
...
int Fira2v2Vision::Judge_Basket_legal(cv::Mat& src)
{
...
double Fira2v2Vision::Calc_Similar(cv::Mat& src)
{
...
cv::Mat Fira2v2Vision::Basket_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void Fira2v2Vision::Not_Find_Basket()
{
    basket_area = -1;
    basket_length = -1;
...
//void Fira2v2Vision::imageProcess(cv::Mat image,ImgProcResult *Result)
void Fira2v2Vision::Process_Ball(cv::Mat image)
{
...
int Fira2v2Vision::Judge_Ball_Legal(Vec3f circle)
{
...
int Fira2v2Vision::Judge_Ball_Color_Legal(cv::Mat& src)
{
...
cv::Mat Fira2v2Vision::Ball_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void Fira2v2Vision::Process_Ball_2(Mat image)
{
    image = channel[0].clone();

	ball_project_result = image;
...
void Fira2v2Vision::Find_Ball_center(cv::Mat src)
{
...
int Fira2v2Vision::Judge_Ball_Legal_2(cv::Mat &src)
{
...
void Fira2v2Vision::Not_Find_Ball()
{
...
//--------------------------------------------------------------------------
//**************************************************************************
void Fira2v2Vision::Set_parameter()
{
...
void Fira2v2Vision::Load_parameter()
{
//read
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/Archery/test_img/2.txt");
    ifstream ifile("../source/data/set-fira_2v2-parameter/3.txt");
    if(!ifile){
        cerr<<"erroe"<<endl;
    }
    string lineword;
...
void Fira2v2Vision::Store_parameter()
{
//write
