//**************************************************************************
//--------------------------------------------------------------------------
// Set GolfVision Paramenter by HSV
//
// Hao Zeng
// version 1.0
// Date    2017.9.19
 ...
 *      2.确认头文件和cpp里面Imgprocess函数是否更改 void GolfVision::imageProcess(cv::Mat image, ImgProcResult *Result)
 *      3.确认头文件中的类名是否更改class GolfVision : public ImgProc
 *      4.确认类中是否增加成员变量GolfVisionResult MyResult;
 *      5.确认头文件中是否增加class GolfVisionResult : public ImgProcResult
 *      6.确认imgprocess函数中是否给Result类返回参数信息
 *
 *        MyResult.center.x = center_cols;
 *        MyResult.center.y = center_rows;
 *        MyResult.valid    = false;
 *        (*dynamic_cast<GolfVisionResult*>(Result)) = MyResult;
 *
 *      7.是否在imgprocess函数末尾增加 (*dynamic_cast<GolfVisionResult*>(Result)) = MyResult;
 *      8.是否更改了调试时图片的输出路径
...
/******Public******/
void GolfVision::imageProcess(cv::Mat image, ImgProcResult *Result)
//void GolfVision::imageProcess(cv::Mat image)
{
    ball_setNotFound();
    hole_setNotFound();
...
void GolfVision::Pretreat(cv::Mat image)
{
    //basket
    src_img = image.clone();
...
void GolfVision::Dilate_img(Mat src, Mat &dst, int size)
{
    Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    dilate(src, dst, element, Point(-1, -1), 1);
}

void GolfVision::Erode_img(cv::Mat src, cv::Mat &dst, int size)
{
    cv::Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    erode(src, dst, element, Point(-1, -1), 1);
}

cv::MatND GolfVision::getHistogram(const cv::Mat &image)
{
    cv::MatND hist;//创建直方图
    cv::calcHist(//计算直方图
        &image,//待计算的图像
...
cv::Mat GolfVision::getHistogram_Image(const cv::Mat &image)
{
...
void GolfVision::Write_img(cv::Mat& src,  const char floder_name[30], int num)
{
...
void GolfVision::Process_hole(cv::Mat image)
{
...
void GolfVision::Find_hole_center(cv::Mat src)
{
...
int GolfVision::Judge_hole_legal(cv::Mat& src)
{
...
cv::Mat GolfVision::hole_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void GolfVision::hole_setNotFound()
{
    hole_center_rows = hole_center_cols = -1;
...
void GolfVision::Process_ball(cv::Mat image)
{
    image = channel[0].clone();
...
void GolfVision::Find_ball_center(cv::Mat src)
{
...
int GolfVision::Judge_ball_legal(cv::Mat& src)
{
...
cv::Mat GolfVision::ball_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void GolfVision::ball_setNotFound()
{
    ball_center_rows = ball_center_cols = -1;
...
int GolfVision::Judge_centerInRect(Point2f center, Rect rect)
{
...
double GolfVision::Count_possible(Mat bianry_img, Rect roi_rect)
{
...
//new
void GolfVision::Load_parameter()
{
//read
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/GolfVision/test_img/2.txt");
    ifstream ifile("../source/data/set_golf_parameter/3.txt");
    if(!ifile){
        cerr<<"error"<<endl;
    }
    string lineword;
...
void GolfVision::Store_parameter()
{
//write

//    ofstream out("D:/Robot/HuroCup 2017/Prepare/GolfVision/test_img/2.txt");
    ofstream out("3.txt");
    if(out.is_open()){
        printf("is_open\n");
        out << hole_h_min_thre    << " ";
