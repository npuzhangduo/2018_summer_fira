...
//big circle
    ...
    cv::Mat combine_rect_hthre = big_combine.colRange(col_1oce, col_2);
...
*/
//void wy::imageProcess(cv::Mat image)
void wy:: imageProcess(cv::Mat image, ImgProcResult *Result)
{
...
//    MyResult.obj_down = obj_down;
//    MyResult.obj_angle = obj_angle;

    if(obj_center_cols == -1 && obj_center_rows == -1)
    {
            MyResult.obj_valid  = false;
    }
    else
    {
            MyResult.obj_valid  = true;
    }
    (*dynamic_cast<wyResult*>(Result)) = MyResult;

}

void wy::Load_std_img()
{
    //std_big = cv::Mat();
    std_big    = imread("/home/robotis/Desktop/2017 Fira compete(zhang)/source/data/red2.jpg"); 
...
void wy::Pretreat(cv::Mat image)
{
    if (image.empty()){
        std::cout<<"[Warning] the srcImg is emtyp"<<std::endl;
    }
    //basket
    src_img = image.clone();
...
cv::Mat wy::getHistogram_Image(const cv::Mat &image)
{
...
void wy::Write_img(cv::Mat& src,  const char floder_name[30], int num)
{
...
void wy::set_NotFound()
{
    center_rows = center_cols = -1;
    valid       = false;
}
void wy::Process_big(cv::Mat image)
{
...
void wy::Find_big_center(cv::Mat src)
{
...
//////////////////////////////////////
        ...
        if (most_possible != -1)
        {
			...
			obj_up = big_rect.tl().y;
            obj_angle = big_minrect.angle;
            ...
            obj_down = (rect_points[0].y + rect_points[1].y + rect_points[2].y + rect_points[3].y) / 4.0;

       		if (Judge_big_legal( big_rect ) == 0 )
            {
                set_NotFound();
            }
...
int wy::Judge_big_legal(cv::Rect rec)
{
    /*   "/6"-------according to  the size of red paper   */
	/*   the brown:   r----   >100  g------50,100  b------<50 ( camera 341 )  */

	//printf("x:::%lf, y:::%lf, width:::%d\n", obj_center_cols, obj_center_rows ,rec.width);
	float b, g, r ;
	int j = (int)( obj_center_rows + (int)(3 * rec.width/6) ) ;
	int i = (int)obj_center_cols;
	//printf("%d, %d\n", j, i);
	...
	{
		b = src_img.at<Vec3b>(j,i)[0];
		//g = src_img.at<Vec3b>(j,i)[1];
		//r = src_img.at<Vec3b>(j,i)[2];
		//printf("bbb:::%lf\n", b);
    	if (rec.width <= 18 || b <= 50)
		{
        	return 0;
			printf("can not pass the judge\n");
		}
    	else
        	return 1;
	}	
...
cv::Mat wy::big_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
...
void wy::Load_parameter()
{
//read
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/wy/test_img/2.txt");
//    ifstream ifile("3.txt");
	ifstream ifile("../source/data/set_sprint_param/3.txt");
    if(!ifile){
        cerr<<"error"<<endl;
    }
    string lineword;
...
void wy::Store_parameter()
{
//write

//    ofstream out("D:/Robot/HuroCup 2017/Prepare/wy/test_img/2.txt");
    ofstream out("3.txt");
    if(out.is_open()){
        printf("is_open\n");
        out << big_h_min_thre    << " ";
