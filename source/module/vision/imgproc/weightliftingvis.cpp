...
*/
void Weightlift::imageProcess(cv::Mat image, ImgProcResult *Result)
{
printf("\n");
    set_NotFound();
    Load_std_img();
...
printf("num:::%d\n",start_file_num);
    ...
    (*dynamic_cast<WeightliftResult*>(Result)) = MyResult;

}

void Weightlift::Load_std_img()
{
    std_big    = imread("/home/robotis/Desktop/2017.9/source/data/std_img/gun2.png"); //yellow
    cv::cvtColor(std_big, std_big, CV_RGB2HSV);
    cv::split(std_big, std_big_hsv_channel);
    cv::normalize(std_big_hist, std_big, 1.0);
}

void Weightlift::Pretreat(cv::Mat image)
{
    //basket
    src_img = image.clone();
...
void Weightlift::Dilate_img(Mat src, Mat &dst, int size)
{
    Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    dilate(src, dst, element, Point(-1, -1), 1);
}

void Weightlift::Erode_img(cv::Mat src, cv::Mat &dst, int size)
{
    cv::Mat element = getStructuringElement(MORPH_RECT, Size(2 * size + 1, 2 * size + 1), Point(size, size) );
    erode(src, dst, element, Point(-1, -1), 1);
}

cv::MatND Weightlift::getHistogram(const cv::Mat &image)
{
    cv::MatND hist;//创建直方图
    cv::calcHist(//计算直方图
        &image,//待计算的图像
...
cv::Mat Weightlift::getHistogram_Image(const cv::Mat &image)
{
...
void Weightlift::Write_img(cv::Mat& src,  const char floder_name[30], int num)
{
...
void Weightlift::set_NotFound()
{
    center_rows = center_cols = -1;
    valid       = false;
}
void Weightlift::Process_big(cv::Mat image)
{
...
void Weightlift::Find_big_center(cv::Mat src)
{
...
//                obj_angle = big_minrect.angle;
				if(big_minrect.size.width > big_minrect.size.height)
					obj_angle = -(big_minrect.angle);
				else if(big_minrect.size.width < big_minrect.size.height)
					obj_angle = -( 90 + big_minrect.angle);
				printf("angle:::%lf\n", obj_angle);
...
int Weightlift::Judge_big_legal(cv::Mat& src)
{
    if (src.rows <= 2 || src.cols <= 10)
        return 0;
    else
        return 1;
}
cv::Mat Weightlift::big_BackProject(const cv::MatND& hist, const cv::Mat& findimage)
{
    ...
    Dilate_img(result, result, 3);
    return result;
}
void Weightlift::Load_parameter()
{
//read
//    ifstream ifile("/home/zenghao/HuroCup 2017/Prepare/weightlift/test_img/2.txt");
//    ifstream ifile("3.txt");
	ifstream ifile("../source/data/set_weightlift_parameter/3.txt");
    ...
    printf("big_h_min_thre:%d; big_h_max_thre:%d \n", big_h_min_thre, big_h_max_thre);
	printf("big_s_thre:%d\n", big_s_thre);
}

void Weightlift::Store_parameter()
{
//write

//    ofstream out("D:/Robot/HuroCup 2017/Prepare/weightlift/test_img/2.txt");
    ofstream out("3.txt");
    if(out.is_open()){
        printf("is_open\n");
        out << big_h_min_thre    << " ";
